...
def test_derive_native_script_hash(
    ...
    _deriveNativeScriptHash_addScript(
        firmware, navigator, client, testCase.script, False
    )
...
def _deriveNativeScriptHash_addScript(
    ...
    if script.type in [
        ...
        _deriveScriptHash_startComplexScript(
            firmware, navigator, client, script, complex_nav
        )
        assert isinstance(
        ...
        for subscript in script.params.scripts:
            _deriveNativeScriptHash_addScript(
                firmware, navigator, client, subscript, True
            )
    else:
    ...
    with client.derive_script_finish(testCase.displayFormat):
        if firmware.is_nano:
            ...
            if testCase.script.type in (
                ...
                testCase.displayFormat != NativeScriptHashDisplayFormat.POLICY_ID
                and not (
                    testCase.script.type == NativeScriptType.N_OF_K
                    and testCase.script.params.requiredCount > 0
                )
            ):
                moves += [NavInsID.RIGHT_CLICK]
            moves += [NavInsID.BOTH_CLICK]
...
def test_derive_native_script_hash_reject(
    firmware: Firmware,
    backend: BackendInterface,
    navigator: Navigator,
    scenario_navigator: NavigateWithScenario,
    ...
    with pytest.raises(ExceptionRAPDU) as err:
        # Send the APDU
        test_derive_native_script_hash(
